...
app.post('/getRecord',  (req, res) => {
	console.log('add');
	let {startTime='' , endTime='',reason=''} = req.body
	let sql = `select * from account_record where account_time > ${startTime} and account_time < ${endTime} and account_reason LIKE '%${reason}%'  order by  account_time desc `
	conn(sql).then(row=>{
		if(row){
...
app.post('/addRecord',  (req, res) => {
	...
	let sql = `INSERT INTO account_record (account_reason,account_time,account_money) VALUES ( '${reason}',  ${time} , ${money})`
	conn(sql).then(row=>{
		if(row){
...
app.post('/editRecord',  (req, res) => {
	let {money ,time, reason='',desc='',id} = req.body
	let sql = `UPDATE  account_record set account_reason = ${'\''+reason+'\''},account_money = ${'\''+money+'\''} where id = ${id}`
	conn(sql).then(row=>{
		if(row){
